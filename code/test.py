...
def get_modal_results(model, num_modes=15, group_name="ALL"):
    """
    ...
    model.Results.Setup.SetCaseSelectedForOutput("MODAL")
    ...
    ret = model.Results.ModalPeriod(NumberResults, LoadCase, StepType, StepNum, Period, Frequency, CircFreq, EigenValue)
    if ret[-1] != 0:
    ...
    ret2 = model.Results.ModeShape(group_name, GroupElm, NumberResults2, Obj, Elm, LoadCase2, StepType2, StepNum2, U1, U2, U3, R1, R2, R3)
    print("ret2的长度为:", len(ret2))
    print(f"ret2的关键字：{ret2[1][0:10]}...")  # 打印前10个字符
